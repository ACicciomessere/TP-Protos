...
// Estructura para la respuesta "genérica"
typedef struct {
  int success;
...
int mgmt_send_command(int sock, mgmt_command_t cmd, const char *username,
                      const char *password);
int mgmt_receive_response(int sock, mgmt_response_t *response);
void mgmt_close_connection(int sock);
/* mgmt_accept_loop eliminado - management es ahora 100% no bloqueante */
...
void mgmt_update_user_stats(const char *username, uint64_t bytes_transferred,
