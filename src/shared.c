...
/* =========================================================================
 * Configuración dinámica del servidor (en memoria de proceso)
 * ========================================================================= */
...
static bool g_dissectors_enabled    = true;   // Dissectores habilitados
...
static void save_users_to_file(void) {
    ...
    if (!f) {
        perror("Opening users file for write");
        return;
    }
...
/* =========================================================================
 * Helpers sobre usuarios / stats (internos)
 * ========================================================================= */

// IMPORTANTE: esta función asume que EL LLAMADOR YA TOMÓ users_mutex.
static int find_user(const char* username) {
    if (g_shared_data == NULL || username == NULL) {
        return -1;
    }

    for (int i = 0; i < g_shared_data->user_count; i++) {
        if (g_shared_data->users[i].active &&
            strcmp(g_shared_data->users[i].username, username) == 0) {
            return i;
        }
    }
    return -1;
...
// Agregar usuario
static int add_user(const char* username, const char* password) {
    if (g_shared_data == NULL || username == NULL || password == NULL) {
        return -1;
    }
...
static int get_users(user_t* user_list, int max_users) {
    if (g_shared_data == NULL || user_list == NULL || max_users <= 0) {
        return 0;
    }
...
// Copiar estadísticas globales
static void get_stats(stats_t* stats) {
    if (g_shared_data == NULL || stats == NULL) return;
...
/* =========================================================================
 * Actualización de estadísticas globales / por usuario
 * ========================================================================= */
...
void mgmt_update_stats(uint64_t bytes_transferred, int connection_change) {
    ...
    if (connection_change > 0) {
        ...
        if (g_shared_data->stats.current_connections >
            g_shared_data->stats.peak_concurrent_connections) {
            g_shared_data->stats.peak_concurrent_connections =
                g_shared_data->stats.current_connections;
        }
    } else if (connection_change < 0) {
        if (g_shared_data->stats.current_connections > 0) {
            g_shared_data->stats.current_connections--;
        }
    }
...
void mgmt_update_user_stats(const char* username,
    ...
    if (connection_change > 0) {
        ...
        if (user_stats->current_connections > 0) {
            user_stats->current_connections--;
        }
...
int mgmt_send_command(int sock, mgmt_command_t cmd,
...
/* =========================================================================
 * Helpers de respuestas específicas
 * ========================================================================= */
