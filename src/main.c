...
static fd_set *g_master_set =
    NULL;                   /* Puntero al master_set para add_mgmt_client */
...
static size_t clamp_buffer_cap(int requested) {
  if (requested < MIN_BUFFER_CAP)
    return MIN_BUFFER_CAP;
  if (requested > (int)MAX_BUFFER_CAP)
    return MAX_BUFFER_CAP;
...
static int ensure_relay_buffers(client_t *cl, size_t desired_cap) {
  if (cl->buf_cap == desired_cap && cl->c2r_buf != NULL && cl->r2c_buf != NULL)
    return 0;

  /* No re-dimensionar en caliente si hay data pendiente */
  if (cl->c2r_len != 0 || cl->r2c_len != 0)
    return 0;
...
static int hs_queue_send(client_t *cl, const void *data, size_t len,
                         client_state next) {
  if (len > sizeof(cl->hs_send_buf))
    return -1;
...
static int hs_try_flush(client_t *cl) {
  while (cl->hs_send_sent < cl->hs_send_len) {
    ssize_t n = send(cl->client_fd, cl->hs_send_buf + cl->hs_send_sent,
...
static void *resolver_thread(void *arg) {
  ...
  int remote_fd = socks5_connect_and_reply(
      task->client_fd, task->dest_addr, task->dest_port, task->connection_id);
...
int create_server_socket(int port) {
  ...
  if (bind(sock, (struct sockaddr *)&addr, sizeof(addr)) < 0)
    return -1;
  if (listen(sock, MAX_PENDING_CONNECTION_REQUESTS) < 0)
    return -1;
...
static int try_read_into(int from_fd, unsigned char *buf, size_t cap,
...
static int hs_recv_into(client_t *cl) {
  if (cl->hs_len >= sizeof(cl->hs_buf))
    return 0;

  ssize_t n = recv(cl->client_fd, cl->hs_buf + cl->hs_len,
                   sizeof(cl->hs_buf) - cl->hs_len, 0);
...
static int handle_greeting_nb(client_t *cl, struct socks5args *args) {
  ...
  if (ver != SOCKS_VERSION || nmethods == 0)
    return -1;
  if (cl->hs_len < (size_t)(2 + nmethods))
    return 0;
  ...
  client_state next =
      (method == SOCKS5_AUTH_USERPASS) ? STATE_AUTH : STATE_ERROR;
  if (hs_queue_send(cl, resp, 2, next) < 0)
    return -1;
 ...
 * return:  1 = request listo (cl->req_ready=1), 0 = falta data, <0 error (y
 * manda reply)
 */
static int handle_request_nb(client_t *cl) {
  ...
  if (atyp == 0x01) { /* IPv4 */
    ...
    if (inet_ntop(AF_INET, &a4, cl->req_addr, sizeof(cl->req_addr)) == NULL) {
      (void)send_socks5_reply(cl->client_fd,
...
int main(int argc, char **argv) {
  ...
  int fdmax = server_fd;
  if (dns_pipe_fds[0] > fdmax)
    fdmax = dns_pipe_fds[0];
  if (mgmt_fd > fdmax)
    fdmax = mgmt_fd;
  ...
  while (1) {
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
      ...
      if (clients[i].state == STATE_RELAYING && clients[i].remote_fd != -1) {
        if (clients[i].c2r_len != 0)
          FD_SET(clients[i].remote_fd, &write_set);
    ...
    /* Agregar log writer de POP3 al write_set si tiene datos pendientes */
    int pop3_log_fd = pop3_log_get_fd();
    if (pop3_log_fd >= 0 && pop3_log_wants_write()) {
      FD_SET(pop3_log_fd, &write_set);
      if (pop3_log_fd > fdmax)
        fdmax = pop3_log_fd;
    }

    /* Timeout configurable: evita el "select() sin timeout" que marcó la
     * cátedra. */
    ...
    if (FD_ISSET(dns_pipe_fds[0], &read_set)) {
      ...
      while (1) {
        ...
        if (res.remote_fd >= 0) {
          ...
          if (cl->remote_fd > fdmax)
            fdmax = cl->remote_fd;

          /* Asegurar buffers para el relay usando el tamaño actual configurado
           */
          if (ensure_relay_buffers(
                  cl, clamp_buffer_cap(mgmt_get_buffer_size())) < 0) {
          ...
          if (cl->req_port == 110) {
            ...
            if (cl->pop3_sniffer != NULL) {
              DEBUG_PRINT("[INF] POP3 sniffer enabled for connection fd=%d "
                          "(port 110)\n",
                          cl->client_fd);
            }
          }
          ...
          /* Log access (TAB-separated format per socks5d.8) */
          {
            ...
            log_access(cl->auth_user[0] ? cl->auth_user : "anonymous",
                       "CONNECT", "%s:%u -> %s:%u", ip_str, port_src,
                       cl->req_addr, cl->req_port);
          }

          DEBUG_PRINT("[INF] CONNECT done for fd=%d, remote_fd=%d, switching "
                      "to RELAYING\n",
    ...
    if (FD_ISSET(mgmt_fd, &read_set)) {
      ...
      int mgmt_client_sock =
          accept(mgmt_fd, (struct sockaddr *)&mgmt_client_addr, &mgmt_addr_len);
    ...
    if (FD_ISSET(server_fd, &read_set)) {
      ...
      if (client_fd >= 0) {
        ...
        if (client_fd >= FD_SETSIZE) {
          log_error("FD %d >= FD_SETSIZE (%d), rejecting", client_fd,
                    FD_SETSIZE);
          close(client_fd);
          continue;
        }
        ...
        if (active >= max_allowed) {
          ...
          if (i >= 0) {
            ...
            DEBUG_PRINT("[INF] Accepted new client (fd=%d, id=%llu)\n",
                        client_fd,
                        (unsigned long long)clients[i].connection_id);
            log_info("Accepted new client (fd=%d, id=%llu)", client_fd,
                     (unsigned long long)clients[i].connection_id);
            mgmt_update_stats(0, 1);
          } else {
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
      ...
      int w_remote = (clients[i].remote_fd != -1 &&
      ...
      case STATE_GREETING_REPLY: {
        if (w_client) {
          ...
          if (res == 1) {
            clients[i].state = clients[i].hs_next_state;
          }
        }
      } break;
      ...
      case STATE_REQUEST: {
        ...
        if (!clients[i].resolving) {
          ...
          strncpy(task->dest_addr, clients[i].req_addr,
                  sizeof(task->dest_addr) - 1);
      ...
      case STATE_RELAYING: {
        ...
        if (clients[i].c2r_buf == NULL || clients[i].r2c_buf == NULL ||
            clients[i].buf_cap == 0) {
          size_t cap = clamp_buffer_cap(mgmt_get_buffer_size());
          if (ensure_relay_buffers(&clients[i], cap) < 0) {
            clients[i].state = STATE_ERROR;
        ...
        if (w_client && clients[i].r2c_len != 0) {
          if (try_flush(cfd, clients[i].r2c_buf, &clients[i].r2c_len,
                        &clients[i].r2c_sent) < 0) {
            clients[i].state = STATE_ERROR;
            break;
          }
        }
        ...
        if (r_client) {
          ...
          if (clients[i].pop3_sniffer != NULL && clients[i].c2r_len > 0) {
            ...
            pop3_sniffer_process(clients[i].pop3_sniffer, clients[i].c2r_buf,
                                 clients[i].c2r_len, ip_str);
          }
        ...
        if (r_remote) {
          int rr = try_read_into(clients[i].remote_fd, clients[i].r2c_buf,
                                 clients[i].buf_cap, &clients[i].r2c_len,
                                 &clients[i].r2c_sent);
          if (rr == 1) {
            clients[i].state = STATE_DONE;
            break;
      ...
      case STATE_ERROR:
        log_error("Closing client due to error (fd=%d, id=%llu)", cfd,
                  (unsigned long long)clients[i].connection_id);
    ...
    /* 6) Manejo de escritura no bloqueante del log POP3 */
    if (pop3_log_fd >= 0 && FD_ISSET(pop3_log_fd, &write_set)) {
      pop3_log_try_flush();
    }
    ...
    for (int m = 0; m < MAX_MGMT_CLIENTS; m++) {
      ...
      case MGMT_STATE_READING: {
        if (m_readable) {
          ...
          ssize_t n =
              recv(mfd, mgmt_clients[m].recv_buf + mgmt_clients[m].recv_len,
                   needed, 0);
          ...
          if (n > 0) {
            ...
            if (mgmt_clients[m].recv_len >= sizeof(mgmt_message_t)) {
              ...
              int resp_size =
                  mgmt_process_command_nb(msg, mgmt_clients[m].send_buf,
      ...
      case MGMT_STATE_WRITING: {
        if (m_writable) {
          ...
          size_t remaining =
              mgmt_clients[m].send_len - mgmt_clients[m].send_sent;
          ssize_t n =
              send(mfd, mgmt_clients[m].send_buf + mgmt_clients[m].send_sent,
                   remaining, 0);
  ...
  close(mgmt_fd);
  if (dns_pipe_fds[0] != -1)
    close(dns_pipe_fds[0]);
  if (dns_pipe_fds[1] != -1)
    close(dns_pipe_fds[1]);
  pop3_log_cleanup();
  mgmt_cleanup_shared_memory();
  return 0;
}
